...
public class ToolCallAgent extends ReActAgent{
    ...
    //工具调用管理者
    private ToolCallingManager toolCallManager;
    ...
    public boolean think() {
        ...
        try {
            ...
            String toolCallInfo  = toolCallList.stream().map(toolCall -> String.format("工具名称：%s，参数：%s",
                            toolCall.name(),
                            toolCall.arguments()))
            ...
            if(toolCallList.isEmpty()) {
                ...
                // 需要调用工具时，无需记录只助手消息，因为调用工具时会自动记录
                return true;
            }
        } catch (Exception e) {
    ...
    public String act() {
        ...
        //判断是否代用了终止工具
        boolean b = toolResponseMessage.getResponses().stream().anyMatch(response -> "doTerminate".equals(response.name()));
        if (b) {
